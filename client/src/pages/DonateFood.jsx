...
import { UtensilsCrossed, MapPin, Phone, Calendar, Image as ImageIcon } from 'lucide-react'; // Added icons
  ...
  return (
    <div className="page-container bg-gradient-to-br from-green-50 to-blue-50 min-h-screen flex items-center justify-center p-4">
      ...
      <div className="bg-white p-8 md:p-10 rounded-xl shadow-2xl w-full max-w-lg">
        <h2 className="text-3xl md:text-4xl font-extrabold text-center text-green-700 mb-8">Donate Food</h2>
        <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
          <div className="relative">
            <UtensilsCrossed className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
            <input
              type="text"
              placeholder="Food Name (e.g., Cooked Rice, Bread)"
              value={name}
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Quantity (e.g., 2 kg, 10 servings)"
              value={quantity}
          ...
          <label className="block text-gray-700 font-medium text-left flex items-center">
            <Calendar className="w-5 h-5 mr-2 text-gray-500" /> Expected Edible Till Date:
            <input
              type="date"
              value={expectedEdibleTill}
              onChange={(e) => setExpectedEdibleTill(e.target.value)}
              required
